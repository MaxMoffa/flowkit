...
- a **Preset selector** (`odori`, `feedback`, `restaurant`, plus a few demos: custom
  steps, OAuth+Map, result actions, file step);
- a **Theme selector** (`warm-paper`, `mint-fresh`, `midnight-ink`, `sunset-clay`,
  `rose-quartz`, `scarlet-ink`, plus the `showcase` demo theme);
- a **light/dark toggle**;
